...
export default function Assessment({ unit, onComplete, onQuit }) {
  ...
  const [draggedItem, setDraggedItem] = useState(null); // Now stores the unique 'id' of the bank item
  ...
  const handleDrop = (e, slotId) => {
    ...
    // Prevent same ID existing in multiple slots
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-50 font-sans relative">
      ...
      <div className="flex flex-col md:flex-row flex-1 overflow-hidden relative z-10">
        ...
        {/* Left Hand Side (Passage & Graphics) - Only renders if they exist */}
        {hasLHS && (
          <div className="w-full md:w-1/2 p-6 md:p-8 overflow-y-auto border-b md:border-b-0 md:border-r border-slate-200 bg-white z-10 pb-32">
            {hasPassage && (
              <div className={`animate-in fade-in duration-300 ${hasGraphic ? 'mb-8 border-b-2 border-slate-100 pb-8' : ''}`}>
            ...
            {hasGraphic && (
              <div className="w-full animate-in fade-in duration-300">
                {currentQuestion.inlineSvg ? (
                  <div 
                    className="w-full h-auto flex items-center justify-center p-6 rounded-3xl bg-slate-50 border-2 border-slate-200 shadow-inner"
                  ...
                  <img 
                    ...
                    className="w-full h-auto object-contain rounded-3xl bg-slate-50 border-2 border-slate-200 shadow-inner p-6"
                  />
                )}
              </div>
            )}
        ...
        {/* Right Hand Side (Questions & Controls) */}
        <div className={`w-full ${hasLHS ? 'md:w-1/2 bg-slate-50' : 'max-w-4xl mx-auto bg-white border-x border-slate-200'} flex flex-col relative overflow-hidden pb-48`}>
                  ...
                  <div className="bg-white border-2 border-dashed border-slate-300 rounded-3xl p-5 shadow-sm">
                    ...
                    <div className="flex flex-wrap gap-3">
                      {(currentQuestion.bank || []).map((opt) => {
                        // Check if the unique ID is already used
                ...
                <div>
                  ...
                  <div className={`grid gap-4 ${(currentQuestion.targets || []).length > 1 && currentQuestion.type !== 'order' ? 'grid-cols-1 xl:grid-cols-2' : 'grid-cols-1'}`}>
                    {(currentQuestion.targets || []).map((target) => {
                      ...
                      return (
                        <div key={target.id} className="flex flex-col bg-white p-4 rounded-3xl border-2 border-slate-200 shadow-sm">
                          <div className="font-bold text-slate-800 mb-3 text-[15px] leading-tight text-center border-b-2 border-slate-100 pb-3">{target.title}</div>
                          <div className="flex flex-col gap-2">
                            {Array.from({ length: requiredCount }).map((_, slotIndex) => {
                              ...
                              let slotStyle = "bg-slate-50 border-dashed border-slate-300 text-slate-400";
                              
                              if (testPhase === 'reviewing') {
                                const expectedArr = currentQuestion.correctSets?.[target.id] || [];
                                // For evaluation, compare the 'val' property, not the 'id'
                                ...
                                if (!droppedId) slotStyle = "bg-[#FFE5E5] border-[#EA4335] border-solid text-[#A32D23] shadow-sm";
