...
| [ext-math/notes-and-widgets.md](ext-math/notes-and-widgets.md) | the deck: slide beats, the `venn` activity, the three widgets |
...
| Unit id | `EM_<assignment number>` — `EM_06` is Assignment 06. Zero-padded, so the folder sorts in course order. |
...
| `practice` — **Gate 1: Apply** | 15 | one production task per skill the assignment tests (25–30 each) · `WORKBOOK` 15 | ~100 |
| `mastery` — **Gate 2: Quiz & Arcade** | 60 | `ASSESSMENT` 20 · `GAMES` 0 | 20 |

`EM_06` totals 140 XP against a 100 XP unit. Over-provision is deliberate: a student can
drop a whole task and still finish. **Re-derive the gates whenever the task mix changes** —
a gate must sit at or below **80% of the XP available before it**, and `npm run validate`
fails otherwise ([ged-unit-shape.md](ged-unit-shape.md) §2).
...
options are: extend an engine, write one (§6 of the task-engines doc), or leave it in the
...
alone, and reads a bare `√12` as "root 12".
...
conjugate pair. Answers must be an integer or Yes/No; **verify a new generator against an
